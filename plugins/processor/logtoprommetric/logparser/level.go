...
func GuessLevelAndException(line string) (level Level, exception string) {
	exceptionType := ""
	...
	if len(fields) == 0 {
		return LevelUnknown, exceptionType
	}
	limit := len(fields)
	...
	for _, f := range fields[:limit] {
		...
		for _, sf := range subfields {
			sfLower := strings.TrimLeft(strings.ToLower(sf), "\"[(<'")
			if strings.HasSuffix(sfLower, "exception") {
				exceptionType = sf
			}
			sf = sfLower
			...
			if l := len(sf); l == 3 {
				switch sf {
				case "dbg", "trc":
					return LevelDebug, exceptionType
				case "inf":
					return LevelInfo, exceptionType
				case "wrn":
					return LevelWarning, exceptionType
				case "err":
					return LevelError, exceptionType
				case "ftl":
					return LevelCritical, exceptionType
				}
			} else if l >= 4 {
				switch sf[:4] {
				case "debu":
